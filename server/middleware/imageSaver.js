    ...
    if (req.granted) {
        if (!req.files || Object.keys(req.files).length === 0) {
            ...
            if (req.files.logo && req.files.logo[0]) {
                ...
                if (this.save(req.files.logo[0].path, fileName, path)) {
            ...
            if (req.files.background && req.files.background[0]) {
                ...
                let path = "teams/";
                if (this.save(req.files.background[0].path, fileName, path)) {
			...
			if (req.files.subscription && req.files.subscription[0]) {
                ...
                if (this.save(req.files.subscription[0].path, fileName, path)) {
            ...
            if (req.files.banner && req.files.banner[0]) {
                ...
                let path = "games/";
                if (this.save(req.files.banner[0].path, fileName, path)) {
            ...
            if (req.files.avatar && req.files.avatar[0]) {
                ...
                let path = "users/";
                if (this.save(req.files.avatar[0].path, fileName, path)) {
